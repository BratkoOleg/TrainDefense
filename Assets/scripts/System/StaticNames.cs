public class StaticNames
{
    public static string LootSpawner = "LootSpawner";
    public static string EnemySpawnSpots1 = "EnemySpawnSpots1";
    public static string LootSpawnSpots1 = "LootSpawnSpots1";

    public static string EnemySpawner = "Spawner";
    public static string Player = "FirstPersonController";
    public static string ObjectSpawner = "ObjectSpawner";

    public static string ObjectPool = "ObjectPool";
    public static string FinishScreen = "FinishedScreen";
    public static string IconInventory = "icon";

    public static string SwordObject = "SwordItem";

    public static string PlayerTag = "Player";
    public static string PlayerHandTag = "ItemSlot";
    public static string TrainTag = "Train";
}
